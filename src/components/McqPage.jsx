...
function McqPage() {
  ...
  return (
    <>
      ...
      <div className='w-full md:w-11/12 flex flex-col lg:flex-row flex-wrap justify-evenly items-center gap-8'>
        {
            qes.categories.map((que, index)=>{
                return(
                <div key={index} className='testcard relative overflow-hidden w-11/12 md:w-full lg:w-[47%] h-auto p-4 flex  justify-around border-2 border-x-primary border-y-bluelg hover:border-x-bluelg hover:border-y-primary shadow-[0_1px_8px_2px] shadow-shadbg duration-300 hover:shadow-none rounded-md'>
                    ...
                    <div className=' flex justify-around items-center text-xl'>
                        <p className='bg text-textlg rounded-full p-2 z-10 relative'>{icons[que.category] || <FaCode size={150}  />}</p>
                    </div>
                </div>
                )
            })
